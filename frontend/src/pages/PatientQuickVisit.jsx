...
import { patientsApi, therapiesApi, assessmentsApi, remindersApi } from "../lib/api";
import { Card } from "../components/ui/card";
import { Button } from "../components/ui/button";
import { Badge } from "../components/ui/badge";
import { ArrowLeft, Stethoscope, Pill, Bell, Sparkles, ChevronRight, FlaskConical, FileText } from "lucide-react";
...
export default function PatientQuickVisit() {
  ...
  const load = async () => {
    try {
      ...
      setPatient(p);
      const t = await therapiesApi.listByPatient(id).catch(() => []);
      setTherapies(t);
      const all = await assessmentsApi.listByPatient(id).catch(() => []);
    ...
    <div className="bg-[#F9FAFB] min-h-screen pb-24">
      ...
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        ...
        <Card className="border-gray-200 p-4 space-y-2">
          ...
          <div className="border-t border-gray-100 pt-2 mt-2 grid grid-cols-2 gap-2">
            <div>
            ...
            <Button
              ...
              className="border-violet-300 text-violet-700 hover:bg-violet-50 w-full"
              data-testid="quick-lab-import"
            >
              <FlaskConical className="w-3.5 h-3.5 mr-1.5" /> Importa lab (PDF/foto)
            </Button>
          </div>
        </Card>
