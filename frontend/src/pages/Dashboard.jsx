...
import {
  ...
  Users, ClipboardList, Loader2, StickyNote, User as UserIcon,
  Lock, Zap, Trash2,
} from "lucide-react";
import { INDEX_LABELS } from "../lib/clinimetrics";
import { RecallBadge } from "../components/patient/RecallFlagControl";
import QuickTaskDialog from "../components/reminders/QuickTaskDialog";
...
// ─────────────────────────────────────────────────────────────────────────────
// Task row — Clinical To-do List
// ─────────────────────────────────────────────────────────────────────────────
function TaskRow({ task, patient, onToggleComplete, onDelete, busy }) {
  const dl          = deadlineLabel(task.due_date);
  const isAsap      = task.priority === "asap";
  const isStandalone = !task.patient_id;
  const isPrivate   = task.visibility === "private";
  const isOverdue   = task.due_date && daysUntil(task.due_date) < 0 && !task.completed;
  ...
  const innerContent = (
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {isStandalone ? (
          <span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.12em] font-bold text-violet-700 bg-violet-50 border border-violet-200 px-1.5 py-0.5 rounded">
            <StickyNote className="w-2.5 h-2.5" /> Post-it
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.12em] font-bold text-blue-700 bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded">
            <UserIcon className="w-2.5 h-2.5" /> Paziente
          </span>
        )}
        {/* ASAP / urgent flag */}
        {isAsap && (
          <span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.12em] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded">
            <Zap className="w-2.5 h-2.5" /> Urgente
          </span>
        )}
        {/* Private badge */}
        {isPrivate && (
          <span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.12em] font-bold text-purple-700 bg-purple-50 border border-purple-200 px-1.5 py-0.5 rounded">
      ...
      <div className="text-[11px] text-gray-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
        {isStandalone ? (
          <span className="italic text-violet-600/80">Nota clinica autonoma</span>
        ) : patientLabel ? (
          <span className="font-medium text-[#0A2540]/70">{patientLabel}</span>
        ) : (
          <span className="text-gray-400">Paziente rimosso</span>
        )}
        {task.notes && (
          <>
            <span className="text-gray-300">·</span>
            <span className="text-gray-400 truncate max-w-[220px]">{task.notes}</span>
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div
      className={`group flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50/70 transition border-b border-gray-100 last:border-0 ${
        isOverdue ? "bg-red-50/30" : isStandalone ? "bg-violet-50/20" : ""
      }`}
    ...
    >
      ...
      {/* Main content — link only for patient-linked tasks */}
      {!isStandalone && patient ? (
        <Link
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {dl && (
              <span className={`text-[10px] font-mono font-semibold px-2 py-0.5 rounded border ${dl.color}`}>
                {dl.text}
              </span>
            )}
      ...
      <button
        ...
        className="opacity-0 group-hover:opacity-100 transition text-gray-300 hover:text-red-500 flex-shrink-0 mt-1"
        title="Elimina task"
...
export default function Dashboard() {
  ...
  // Sort: overdue first, then by date, completed last
  ...
  const sharedWithMeCount = useMemo(() => {
    if (!me?.id) return 0;
    return recallList.filter(
      (p) =>
        p.recall?.flag === "selected" &&
        p.recall?.set_by !== me.id &&
        Array.isArray(p.recall?.shared_with) &&
        p.recall.shared_with.includes(me.id)
    ).length;
  }, [recallList, me?.id]);
  ...
  const deleteTask = async (task) => {
    ...
    try {
      await remindersApi.remove(task.id);
      setReminders((prev) => prev.filter((t) => t.id !== task.id));
      toast.success("Task eliminato");
    } catch (e) {
      toast.error(e.response?.data?.detail || "Errore eliminazione task");
    }
  };
  ...
  return (
    <div className="space-y-6 fade-in" data-testid="dashboard-cockpit">
      ...
      <div className="flex items-end justify-between flex-wrap gap-2">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-gray-500">Ponte di comando</div>
        ...
        <Card className="lg:col-span-2 border-gray-200 shadow-none overflow-hidden" data-testid="todo-card">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 className="font-heading font-bold tracking-tight flex items-center gap-2 text-[#0A2540] text-[16px]">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-md font-semibold transition ${
                  taskFilter === "active" ? "bg-[#0A2540] text-white" : "text-gray-500 hover:bg-gray-100"
                }`}
              ...
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-md font-semibold transition ${
                  taskFilter === "all" ? "bg-[#0A2540] text-white" : "text-gray-500 hover:bg-gray-100"
                }`}
              ...
              <Button
                ...
                className="bg-[#0A2540] text-white hover:bg-[#051626] h-7 px-3 text-[11px]"
                data-testid="new-task-btn"
              >
                <Plus className="w-3.5 h-3.5 mr-1" /> Nuovo task
              </Button>
            </div>
          </div>

          {/* Legend */}
          <div className="px-5 py-2 border-b border-gray-100 bg-gray-50/50 flex items-center gap-4 flex-wrap">
            <span className="inline-flex items-center gap-1 text-[9px] text-violet-700 font-semibold">
              <StickyNote className="w-3 h-3" /> Post-it clinico
            </span>
            <span className="inline-flex items-center gap-1 text-[9px] text-blue-700 font-semibold">
              <UserIcon className="w-3 h-3" /> Task paziente
            </span>
            <span className="inline-flex items-center gap-1 text-[9px] text-amber-700 font-semibold">
              <Zap className="w-3 h-3" /> Urgente
            </span>
            <span className="inline-flex items-center gap-1 text-[9px] text-purple-700 font-semibold">
          ...
          {visibleTasks.length === 0 ? (
            <div className="px-5 py-12 text-center">
              ...
              <p className="text-[11px] text-gray-400 mt-1">
                Crea un post-it clinico o un task paziente con il pulsante "Nuovo task".
              </p>
              <Button
                size="sm"
                onClick={() => setQuickTaskOpen(true)}
                className="mt-4 bg-[#0A2540] text-white hover:bg-[#051626]"
                data-testid="new-task-empty-btn"
              >
                <Plus className="w-3.5 h-3.5 mr-1" /> Nuovo task
              </Button>
            </div>
        ...
        <div className="space-y-5">
          ...
          <Card className="border-gray-200 shadow-none overflow-hidden" data-testid="recall-card">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-1.5">
              <h3 className="font-heading text-sm font-bold tracking-tight text-[#0A2540] flex items-center gap-1.5 flex-1 min-w-0">
                <Star className="w-3.5 h-3.5 flex-shrink-0" /> Da ricontrollare
              </h3>
              {sharedWithMeCount > 0 && (
                <span
                  className="flex-shrink-0 text-[10px] font-bold bg-yellow-100 text-yellow-800 border border-yellow-300 px-1.5 py-0.5 rounded-full"
                  title={`${sharedWithMeCount} pazient${sharedWithMeCount === 1 ? "e condiviso" : "i condivisi"} con te da un collega`}
                  data-testid="recall-shared-badge"
                >
                  {sharedWithMeCount} con me
                </span>
              )}
            </div>
            {recallList.length === 0 ? (
              ...
              recallList.slice(0, 6).map((p) => {
                const isSharedWithMe =
                  ...
                  Array.isArray(p.recall?.shared_with) &&
                  p.recall.shared_with.includes(me?.id);
                const senderFirstName = isSharedWithMe
                  ? (p.recall.set_by_name || "").split(" ")[0] || "collega"
                  : null;
                const rightContent = isSharedWithMe
                  ? <span className="text-[10px] text-yellow-700 font-medium whitespace-nowrap">da {senderFirstName}</span>
                  : (p.recall?.set_at ? new Date(p.recall.set_at).toLocaleDateString("it-IT", { day: "2-digit", month: "2-digit" }) : null);
                return (
                  <PatientCompactRow
                    key={p.id}
                    patient={p}
