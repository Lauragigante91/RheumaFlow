...
import { Save, Beaker, Eye, Microscope, FileText, History } from "lucide-react";
...
export default function SjogrenProfileSection({ patient }) {
  ...
  useEffect(() => {
    ...
    setData((prev) => {
      const { patch, applied } = buildEverPositivePatch("sjogren", prev, everPositiveMap);
              ...
              <div key={a.key} className="border border-gray-200 rounded-md p-2.5 space-y-1.5">
                ...
                {a.hasTiter && v.status === "positive" && (
                  <Input
                    ...
                    data-testid={`sjogren-${a.key}-titer`}
                  />
                )}
              </div>
            );
