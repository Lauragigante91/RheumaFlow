...
import { scleroProfileApi, instrumentalExamsApi, assessmentsApi } from "../../lib/api";
...
import {
  Save, Hand, Activity, Wind, Heart, Stethoscope, Bone, Beaker, ChevronDown, ChevronRight, User, History,
} from "lucide-react";
...
// Detect scleroderma diagnosis — accepts either a diagnosi string OR a full
// patient object { diagnosi, diagnosi_secondarie } (same pattern as isRaDiagnosis).
export function isScleroDiagnosis(d) {
  if (!d) return false;
  let str;
  if (typeof d === "object") {
    const parts = [d.diagnosi || ""];
    if (Array.isArray(d.diagnosi_secondarie)) parts.push(...d.diagnosi_secondarie.filter(Boolean));
  ...
  return (
    s.includes("sclerosi sistem") ||
    s.includes("sclerodermia") ||
    s.includes("scleroderma") ||
    s.includes("ssc") ||
    s.includes("vedoss") ||
    s.includes("crest")
  );
...
export default function ScleroProfileSection({ patient }) {
  ...
  const [derived, setDerived] = useState({ mrss: null, capillaroscopy: null, hrct: null, pft: null, echo: null });
  ...
  useEffect(() => {
    ...
    scleroProfileApi.get(patient.id)
      ...
      .catch(() => setProfile({}))
      .finally(() => { profileLoaded.current = true; setLoading(false); });
  }, [patient?.id]);

  useEffect(() => {
    if (!profileLoaded.current || everPositiveMap.size === 0) return;
    setProfile((prev) => {
      // buildEverPositivePatch for "ssc" expects profileData with antibody and dismissed_antibodies
      const profileData = {
        antibody: prev.antibody || {},
        dismissed_antibodies: prev.dismissed_antibodies || [],
      };
      const { patch, applied } = buildEverPositivePatch("ssc", profileData, everPositiveMap);
      if (applied.length === 0) return prev;
      setAppliedKeys(new Set(applied));
      const merged = mergeEverPositivePatch(profileData, patch);
      return { ...prev, antibody: merged.antibody };
    });
  }, [everPositiveMap, loading]);
  ...
  useEffect(() => {
    ...
    instrumentalExamsApi.list(patient.id)
      .then((exams) => {
        ...
        const latestPft  = sorted.find((e) => e.exam_type === "pft");
        const latestEcho = sorted.find((e) => e.exam_type === "echo_cardiac");
        const latestCap  = sorted.find((e) => e.exam_type === "capillaroscopy");
        const latestHrct = sorted.find((e) => e.exam_type === "hrct");
        const pft = latestPft ? {
          fvc_percent:  latestPft.structured_values?.fvc_percent  ?? null,
          dlco_percent: latestPft.structured_values?.dlco_percent ?? null,
          date: latestPft.exam_date,
          id:   latestPft.id,
        } : null;
        const echo = latestEcho && latestEcho.structured_values?.echo_psap != null ? {
          psap: latestEcho.structured_values.echo_psap,
          date: latestEcho.exam_date,
          id:   latestEcho.id,
        } : null;
        const capillaroscopy = latestCap ? { summary: latestCap.summary, date: latestCap.exam_date, id: latestCap.id } : null;
        const hrct           = latestHrct ? { summary: latestHrct.summary, date: latestHrct.exam_date, id: latestHrct.id } : null;
        setDerived((prev) => ({ ...prev, pft, echo, capillaroscopy, hrct }));
      })
      .catch(() => {});
    assessmentsApi.listByPatient(patient.id)
      .then((items) => {
        const sorted = [...items]
          .filter((a) => a.index_type === "mrss")
          .sort((a, b) => new Date(b.date || b.created_at) - new Date(a.date || a.created_at));
        const latest = sorted[0] || null;
        setDerived((prev) => ({
          ...prev,
          mrss: latest ? { score: latest.score, date: latest.date || latest.created_at, id: latest.id } : null,
        }));
      })
      .catch(() => {});
  }, [patient?.id]);
  ...
  const updateAntibody = (key, value) => {
    setProfile((p) => {
      const dismissed = new Set(p.dismissed_antibodies || []);
      if (value !== "pos" && appliedKeys.has(key)) {
        dismissed.add(key);
      } else if (value === "pos") {
  ...
  const save = async () => {
    ...
    try {
      const data = await scleroProfileApi.upsert(patient.id, {
        ...
        dismissed_antibodies: profile.dismissed_antibodies || [],
      });
      setProfile(data);
  ...
  const sections = [
    { id: "cutaneous", title: "Impegno cutaneo", icon: Hand, render: (props) => renderCutaneous({ ...props, derived }) },
    {
      id: "antibody",
      title: "Profilo anticorpale",
      icon: Beaker,
      render: (props) => renderAntibody({ ...props, appliedKeys, updateAntibody }),
    },
    { id: "vascular", title: "Impegno vascolare", icon: Activity, render: (props) => renderVascular({ ...props, derived }) },
    { id: "ild", title: "Impegno polmonare interstiziale (ILD)", icon: Wind, render: (props) => renderILD({ ...props, derived }) },
    { id: "pah", title: "Ipertensione polmonare (PAH)", icon: Heart, render: (props) => renderPAH({ ...props, derived }) },
...
function DerivedField({ label, value, date, unit, text, noDataText }) {
  const hasData = value != null || !!text;
  const formatted = fmtDate(date);
  return (
    <Field label={label}>
      <div style={{
        padding: "8px 10px", borderRadius: "6px",
        background: hasData ? "#f0fdfa" : "#f8fafc",
        border: `1px solid ${hasData ? "#99f6e4" : "#e2e8f0"}`,
        minHeight: "36px", display: "flex", alignItems: "flex-start",
        justifyContent: "space-between", gap: "8px", flexWrap: "wrap",
      }}>
        {hasData ? (
          <span style={{ fontWeight: 500, fontSize: "13px", color: "#0f766e", flexGrow: 1 }}>
            {text != null
              ? text
              : <>{value}{unit ? ` ${unit}` : ""}
                  {formatted && <span style={{ marginLeft: "8px", fontSize: "11px", color: "#94a3b8" }}>· {formatted}</span>}
                </>
            }
          </span>
        ) : (
          <span style={{ fontSize: "13px", color: "#94a3b8" }}>{noDataText || "— nessun dato nell'archivio esami"}</span>
        )}
        <span style={{ fontSize: "10px", color: "#9ca3af", fontStyle: "italic", marginTop: "2px" }}>sola lettura</span>
      </div>
    </Field>
  );
}
...
function renderCutaneous({ value, set, derived = {} }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="md:col-span-2">
        <DerivedField
          label="mRSS più recente (0-51)"
          value={derived.mrss?.score}
          date={derived.mrss?.date}
          noDataText="— nessun mRSS in archivio (registrare un assessment)"
        />
      </div>
...
function renderAntibody({ value, set, appliedKeys = new Set(), updateAntibody }) {
  const handleChange = updateAntibody || set;
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {ANTIBODIES.map((ab) => (
        <Field
          key={ab.key}
          label={
            ab.type === "tristate" && appliedKeys.has(ab.key) ? (
        ...
        >
          {ab.type === "text" ? (
            ...
            <TriState
              value={value[ab.key]}
              onChange={(v) => handleChange(ab.key, v)}
              testid={`sclero-ab-${ab.key}`}
            />
          )}
        </Field>
      ))}
...
function renderVascular({ value, set, derived = {} }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="md:col-span-2">
        <DerivedField
          label="Capillaroscopia (pattern · data)"
          text={derived.capillaroscopy?.summary}
          noDataText="— nessuna capillaroscopia in archivio esami"
        />
      </div>
      <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-1">
...
function renderILD({ value, set, derived = {} }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="md:col-span-2">
        <DerivedField
          label="HRCT (pattern · data)"
          text={derived.hrct?.summary}
          noDataText="— nessuna HRCT in archivio esami"
        />
      </div>
      <DerivedField
        label="FVC % predetto"
        value={derived.pft?.fvc_percent}
        date={derived.pft?.date}
        unit="% pred."
        noDataText="— nessuna spirometria in archivio"
      />
      <DerivedField
        label="DLCO % predetto"
        value={derived.pft?.dlco_percent}
        date={derived.pft?.date}
        unit="% pred."
        noDataText="— nessuna spirometria in archivio"
      />
...
function renderPAH({ value, set, derived = {} }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <DerivedField
        label="PSAP ecocardiogramma (mmHg)"
        value={derived.echo?.psap}
        date={derived.echo?.date}
        unit="mmHg"
        noDataText="— nessun ecocardiogramma in archivio"
      />
