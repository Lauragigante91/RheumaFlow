...
import { Save, Beaker, Bone, Wind, FileText, History } from "lucide-react";
...
function EverPositiveBadge() {
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded px-1.5 py-0.5 ml-1">
...
export default function RaProfileSection({ patient }) {
  ...
  useEffect(() => {
    ...
    setData((prev) => {
      const { patch, applied } = buildEverPositivePatch("ra", prev, everPositiveMap);
  ...
  const setAbStatus = (profileKey, newValue) => {
    setData((prev) => {
      const dismissed = new Set(prev.dismissed_antibodies || []);
      if (newValue !== "positive" && appliedKeys.has(profileKey)) {
        dismissed.add(profileKey);
      } else if (newValue === "positive") {
        dismissed.delete(profileKey);
      }
      return {
        ...prev,
        [profileKey]: newValue,
    ...
    if (newValue === "positive") {
      setAppliedKeys((prev) => { const s = new Set(prev); s.delete(profileKey); return s; });
    }
  };
        ...
        <div className="space-y-3">
          ...
          <div className="border border-gray-200 rounded-md p-3 space-y-2">
            <div className="flex items-center justify-between gap-3">
              <Label className="text-sm font-medium flex items-center gap-1">
                ...
                {appliedKeys.has("rf_status") && <EverPositiveBadge />}
              </Label>
              <TriState
                value={data.rf_status}
                onChange={(v) => setAbStatus("rf_status", v)}
                testid="ra-rf-status"
              />
            </div>
          ...
          <div className="border border-gray-200 rounded-md p-3 space-y-2">
            <div className="flex items-center justify-between gap-3">
              <Label className="text-sm font-medium flex items-center gap-1">
                Anti-CCP / ACPA
                {appliedKeys.has("acpa_status") && <EverPositiveBadge />}
              </Label>
              <TriState
                value={data.acpa_status}
                onChange={(v) => setAbStatus("acpa_status", v)}
                testid="ra-acpa-status"
              />
            </div>
