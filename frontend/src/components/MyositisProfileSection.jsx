...
import { Save, Beaker, Activity, FileText } from "lucide-react";
import { toast } from "sonner";
import { TriState } from "./ui/TriState";

// Anticorpi miosite-specifici (MSA) e miosite-associati (MAA)
const ANTIBODIES = [
...
export default function MyositisProfileSection({ patient }) {
  ...
  const setAb = (key, val) => {
    setData((p) => ({ ...p, antibodies: { ...p.antibodies, [key]: val } }));
  };
              ...
              {g.items.map((a) => (
                <div key={a.key} className="border border-gray-200 rounded-md p-2.5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <TriState value={data.antibodies?.[a.key] || "unknown"} onChange={(v) => setAb(a.key, v)} testid={`myositis-${a.key}`} />
                  </div>
                </div>
              ))}
            </div>
