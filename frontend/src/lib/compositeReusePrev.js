...
export function applyPrevToRa(prevVisit, setters) {
  ...
  // Fall back to joint_exam (created by text import) when no RA composite score is found
  const jointSrc = src || byType.joint_exam;
  setters.setJoints(_rebuildJointsFrom(jointSrc));
...
export function applyPrevToPsa(prevVisit, setters) {
  ...
  // Fall back to joint_exam (created by text import) when no DAPSA is found
  setters.setJoints(_rebuildJointsFrom(dapsa || byType.joint_exam));
