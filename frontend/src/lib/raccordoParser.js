...
const RESTART_VERB_RE = /\b(?:ripresa\s+(?:della\s+)?terapia\s+(?:\w+\s+)?con|ha\s+ripreso\b|ripreso\s+(?:terapia\s+)?con\b|reintrodott[oa]\b|reintroduzione\s+di\b)\b/i;

const DAL_YEAR_EXT_SRC = String.raw`\bdal?\s+(?:(0?\d|1[012])\/|(gennaio|febbraio|marzo|aprile|maggio|giugno|luglio|agosto|settembre|ottobre|novembre|dicembre)\s+)?((19|20)\d{2})\b`;
  ...
  for (const sentence of sentences) {
    ...
    // Groups: m[1]=MM (numeric, e.g. "01"), m[2]=month_name, m[3]=full year
    {
      ...
      while ((m = dalRe.exec(sentence)) !== null) {
        ...
        const yearStr = m[3];
        const month   = mmNum || mmName || "01";
        const dateVal = `${yearStr}-${month}-01`;
        const datePrec = (mmNum || mmName) ? "month_year" : "year";
